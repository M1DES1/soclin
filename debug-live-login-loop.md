...
| B | Sesja `soclin-live.desktop` nie jest widoczna dla `SDDM` mimo obecnosci pliku w `/usr/share/xsessions/`. | High | Low | Confirmed for latest retest: `cat /usr/share/xsessions/soclin-live.desktop` zwraca `No such file or directory`, wiec `SDDM` nie ma gotowego wpisu sesji live. |
...
- Latest retest on the new artifact shows:
- `brak /etc/sddm.conf`
- `cat /etc/sddm.conf.d/99-soclin-live.conf` contains `Session=soclin-live.desktop`
- `cat /usr/share/xsessions/soclin-live.desktop` returns `No such file or directory`
- `journalctl -b -u sddm --no-pager` still reports `Unable to find autologin session entry ""`
...
- First root cause is fixed in the new artifact, but a second issue remains: the live session desktop file is missing at runtime.
- Next evidence needed: check the contents of `/usr/share/xsessions` and the boot-time `/var/log/soclin-sddm-state.log` from the new artifact to verify whether the file was never created or disappears later.
